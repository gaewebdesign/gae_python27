...
#
           ...
           cr.key = k
           cr.date = str(r.date.month) + "/" + str(r.date.day) + "/" + str(r.date.year)
           cr.day = r.weekday
           cr.location = r.location
           hour = r.start.hour
           if( hour >12): hour-=12
           cr.rstart = str(hour) + ":" + str(r.start.minute)

           hour = r.end.hour
           if( hour >12): hour-=12
           cr.rend = str(hour)  + ":" + str(r.end.minute)
           


           if(type(r.owner) is not  types.NoneType):
             p = datastore.Captain.get_by_key_name( r.owner )
       ...
       for r in rList:
         print(r.date , r.day , r.location , r.fname,r.lname)

       template_values = {
                'CourtReservation': rList

        }

       

       path = os.path.join(os.path.dirname(__file__), 'templates', 'administerdb.html')
       self.response.out.write(template.render(path, template_values))       
           

class DelReservationHandler( webapp2.RequestHandler):
    def get(self):
       print("DelReservationHandler")
#       keys = self.request.get_all('keys')
       keys = self.request.POST.getall('keys')
       for key in keys:
         print( key )
         r = db.delete(key)
         print("results = " + str( r)) 
...
#          print("delete", p.date, p.weekday)
    ...
    def get_( self ):
        print("DelReservationHandler() get ")      
        keylist = self.request.get_all('keys')
        print("get keys")
        for key in keylist:
           p = db.get(key)
           print("delete",key, p.date, p.fname)
           print(key)   
          # db.delete(key)

class DelAllReservationHandler( webapp2.RequestHandler):
    def get(self):
         print("DelAllReservationHandler")
         query="select __key__ from CourtTime "
         keys =  db.GqlQuery( query)
         for k in keys:
           p = db.get(k)
           print( p.date )
           db.delete(k)
           self.response.out.write("deleting " + str(k) + "<br>")
#           break
...
app = webapp2.WSGIApplication(
                                     ...
                                     ('/admindb', TestHandler),
                                     ('/del_reservation', DelReservationHandler),

                                     ('/del_all_reservation', DelAllReservationHandler),

                                     ('/admindb', ScheduleHandler ),
