...
# TODO - more efficient way to rid of last (,) comma
  ...
  self.desc  = desc
  self.key   = key/Users/roger/Downloads/python68/googleappengine/ustacalendar/admin.py




class CourtHandler(webapp.RequestHandler):

    def Writeln(self,t):
        self.response.out.write(t+"\n")

    def template(self, month,day,year):
...
#           'Key'       :  k
        ...
        path = os.path.join(os.path.dirname(__file__), 'courts.html')
        self.response.out.write(template.render(path, template_values))


    def get(self,month,day,year):

        self.template( month,day,year)
...
#       By now, use has logged in and selected courts
        ...
        captain_keyname = sess.get(keyname='user')
        captain_keyname = sess.get(keyname='keyname')
        ...
        Today   =  Today + " @" + library.ctime(datetime.datetime.utcnow() - datetime.timedelta(hours=8))
