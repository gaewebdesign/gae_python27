...
def LoginForm(  ):
    ...
    if( sess.get(keyname='user') ):
        site = Host() + "/logout"
        form = ""
        form = form + '<form name="logout" action="'+site +'" method="POST">'
    ...
    site = Host() + "/login"
    form = ""
    form = form + '<form name="login" action="'+site +'" method="POST">'
    form = form +'&nbsp;'
    form = form + '<input type="hidden" name="path" value ="'+ os.environ["PATH_INFO"] +'">'
...
def GetReservationStart ( ):
        ...
        g = tempstore.OpenDate.get_by_key_name(  "key_opendate" )
        if( g == None):
          return None
...
def Holiday( y,m,d):
  ...
  if(y==2013 and m==7 and d==4): return "4th of July"

  if(y==2013 and m==8 and d==9): return "Districts"
  if(y==2013 and m==8 and d==10): return "Districts"
  if(y==2013 and m==8 and d==11): return "Districts"

  if(y==2013 and m==8 and d==23): return "Sectionals"
  if(y==2013 and m==8 and d==24): return "Sectionals"
  if(y==2013 and m==8 and d==25): return "Sectionals"


  if(y==2013 and m==7 and d==29): return 'Week 7'   
